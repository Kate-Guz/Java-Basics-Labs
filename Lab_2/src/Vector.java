public class Vector {
    ...
    public static double lenV1(Vector[] arr) {
        double lenV1 = Math.sqrt(Math.pow(2, arr[0].x1) + Math.pow(2, arr[0].x2) + Math.pow(2, arr[0].x3));
        System.out.println("Длина первого векторов: " + lenV1);
        return lenV1;
    }

    public static double lenV2(Vector[] arr) {
        double lenV2 = Math.sqrt(Math.pow(2, arr[1].x1) + Math.pow(2, arr[1].x2) + Math.pow(2, arr[1].x3));
